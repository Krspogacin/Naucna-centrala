...
@Service
@Slf4j
public class MailTimeService implements JavaDelegate {
    ...
    public void sendRegistrationNotification(final String username, final String email, final ScientificWork scientificWork) throws MailException, MessagingException {
        ...
        try {
            ...
            helper.setSubject("Naučna centrala - isteklo vreme za korekciju");
            final String message = "Pozdrav " + username + ",<br><br>"
                    + "Nažalost obaveštavamo Vas da se rad sa nazivom: "
                    + "<br><br>"
                    + "<h1>" +  scientificWork.getTitle() + "</h1>"
                    + "<br><br>"
                    + "ne može prihvatiti jer je isteklo vreme za korekciju njegovog formata"
                    + "<br><br>"
                    + "S poštovanjem, <br><br>"
                    + "<i> Naučna centrala  </i>";
